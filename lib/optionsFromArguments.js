...
const optionsFromArguments = function optionsFromArguments(args) {
  ...
  if (typeof stringOrOptions == 'string') {
    ...
    if (otherOptions) {
      ...
      options.useAccelerateEndpoint = otherOptions.useAccelerateEndpoint;
    }
  } else {
    if (args.length == 1) {
  ...
  options = fromEnvironmentOrDefault(options, 'useAccelerateEndpoint', 'S3_USE_ACCELERATE_ENDPOINT', false);
